...
public class UserController {
    ...
    @GetMapping("/login")
    public ResponseEntity checkCred(@RequestBody User user){
        return ResponseEntity.status(200).body(userService.logIn(user.getUsername(), user.getPassword()));
    }
    @GetMapping("/byRole/{role}")
    ResponseEntity getAllByRole(@PathVariable String role){
        return ResponseEntity.status(200).body(userService.getAllByRole(role));
    }
    @GetMapping ("/byAge/{age}")
    ResponseEntity getByAge(@PathVariable int age){
